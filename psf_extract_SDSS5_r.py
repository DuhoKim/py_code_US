...
work_dir = data_dir+'r_bench5/'
psf_dir = data_dir+'r_psf5'
...
################################################################
#class_range = 0.7 (SHA)
class_range = 0.9       # Maximum value of CLASS_STAR selecting PSF stars form sex catalog
...
for x in range(0,len(sha_cat)):
	fn = sha_cat['id'][x]+'-r.fits'		# Image file name
	shutil.copy(data_dir+'r/'+fn,work_dir)
	...
	os.system("sex "+fn+" -PIXEL_SCALE "+str(pixel_size)+" -SEEING_FWHM "+str(irac_fwhm)+' -CHECKIMAGE_TYPE BACKGROUND \
		-CHECKIMAGE_NAME bkg_'+fn+' -VERBOSE_TYPE QUIET -MAG_ZEROPOINT '+str(zmag))
	...
	stars = cat[np.where((cat['FLAGS'] == 0) & (cat['CLASS_STAR'] > class_range) & (cat['ELLIPTICITY'] < \
	    ellip_range) & (cat['FWHM_IMAGE'] < fwhm+fwhm_sig) & (cat['FWHM_IMAGE'] > fwhm-fwhm_sig) & (cat['MAG_AUTO'] > mag_limit))]
