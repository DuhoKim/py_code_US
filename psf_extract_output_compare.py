...
out1=ascii.read(data_dir+'bench4/numstars_fit_func_norm_mag.txt',format='rst')
...
h.hist(out1['NUMSTARS'],bins=np.arange(0,15,1),histtype='bar')
...
h3.hist(out1['NORM3'][~np.isnan(out1['NORM3'])],bins=np.arange(0,0.15,0.01),histtype='bar')
h3.set_xlabel('Norm scatter for fitting PSFs')
h3.set_ylabel('Frequency')

h4.hist(out1['MAG3'][~np.isnan(out1['MAG3'])],bins=np.arange(17,24,0.5),histtype='bar')
h4.set_xlabel('Mag of brightest star for fitting PSFs')
h4.set_ylabel('Frequency')


fig.savefig("/Users/dhk/work/data/NGC_IC/SHA/psf_stars_param.pdf")
