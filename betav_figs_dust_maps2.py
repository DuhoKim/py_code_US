...
#for x in range(0,1):
    ...
    if T < 9:
        for ii in range(-5, 5, 1):
            dust_map = 2.5 * np.log10((bv0s_grid[T_idx] + ii * 0.03) / betav_seg)

            dust_map[np.where(betav_seg > (bv0s_grid[T_idx] + ii * 0.03))] = 0.0
            ...
            np.savetxt(work_dir+'ellipse/scratch_grid2/'+name+'_AV_'+str(ii)+'.txt', (rads, iso_mean, iso_med, iso_std))
