...
def isophote(fits,cx,cy,pa,boa,rad_min,rad_max,rad_num):
	...
	pa_rad          = math.radians(pa-90)                                           # convert PA convention to functional form with units
	for ii in range(1,rad_num+1):
		# General Equation of an Ellipse : (x*cos(A) + y*sin(A))^2/a^2 + (x*sin(A)-y*cos(A))^2/b^2 = 1
		ind1    = 1     <       ((yy-cy)*math.cos(pa_rad)+(xx-cx)*math.sin(pa_rad))**2/r[ii-1]**2 + ((yy-cy)*math.sin(pa_rad)-(xx-cx)*math.cos(pa_rad))**2/(r[ii-1]*boa)**2
		ind2    = 1     >       ((yy-cy)*math.cos(pa_rad)+(xx-cx)*math.sin(pa_rad))**2/r[ii]**2 + ((yy-cy)*math.sin(pa_rad)-(xx-cx)*math.cos(pa_rad))**2/(r[ii]*boa)**2
		ind     = ind1 * ind2
		anul    = fits[ind]
	...
	return r[1:], iso_mean, iso_med, iso_std, tot_mean, tot_med
#############################################################################################
...
for x in range(0,1):
	...
	if no_ellip:
		...
		rads, iso_mean, iso_med, iso_std, tot_mean, tot_med = isophote(V_seg[int(j)-thumb:int(j)+thumb,int(i)-thumb:int(i)+thumb],int(i),int(j),pa,boa,2,thumb,11)
		...
		h25 = fig.add_axes([0.843,0.09,0.1,0.1])
		h25.plot(ell['col1'],ell['col11'])
		h25.plot([hlr_pix,hlr_pix],[0,max(ell['col11'])])

#	h26 = fig.add_axes([0.6,0.5,0.5,0.5])
#	img26 = mpimg.imread('/Users/dhk/work/data/NGC_IC/ellipse/pa_ellip/'+name+'.png')
#	h26.imshow(img26)
#	h26.get_xaxis().set_visible(False)
#	h26.get_yaxis().set_visible(False)

	if bv0 != -1:
		h23 = fig.add_axes([0.63,0.0,0.331,0.5])
		img3=h23.imshow(dust_map[int(j)-thumb:int(j)+thumb,int(i)-thumb:int(i)+thumb],cmap='jet',clim=[0,1],origin='lower')
		...
		if no_ellip:
			...
			ellipse1 = Ellipse((thumb,thumb),ell['col1'][hlr_idx],ell['col1'][hlr_idx]*(1.0-ell['col5'][hlr_idx]),ell['col13'][hlr_idx]+90,fill=False)
			ellipse2 = Ellipse((thumb,thumb),ell['col1'][hlr3_idx],ell['col1'][hlr3_idx]*(1.0-ell['col5'][hlr3_idx]),ell['col13'][hlr3_idx]+90,fill=False)
			h23.add_artist(ellipse1)
			h23.add_artist(ellipse2)
		

		######## run ELLIPSE for A_V for 6 Zs and 3.6 ##########
		if bv_fit:
			if no_ellip:
			else:
				if os.path.exists('tmp7.fits'):
					os.remove('tmp7.fits')
			...
			if os.path.exists('tmp1.fits'):
				os.remove('tmp1.fits')
	                ...
	                if os.path.exists('tmp5.fits'):
        	                os.remove('tmp5.fits')
	                if os.path.exists('tmp6.fits'):
        	                os.remove('tmp6.fits')

			if os.path.exists('tmp.tab'):
        	                os.remove('tmp.tab')

			if os.path.exists('tmp1.tab'):
				os.remove('tmp1.tab')
	                if os.path.exists('tmp2.tab'):
        	                os.remove('tmp2.tab')
	                if os.path.exists('tmp3.tab'):
        	                os.remove('tmp3.tab')
	                if os.path.exists('tmp4.tab'):
        	                os.remove('tmp4.tab')
	                if os.path.exists('tmp5.tab'):
        	                os.remove('tmp5.tab')
	                if os.path.exists('tmp6.tab'):
        	                os.remove('tmp6.tab')

			hdu1 = fits.PrimaryHDU(dust_map)
        	        hdu1.writeto('tmp.fits')

			hdu1 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[0]/betav_seg))
			hdu1.writeto('tmp1.fits')
	                hdu2 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[1]/betav_seg))
        	        hdu2.writeto('tmp2.fits')
                	hdu3 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[2]/betav_seg))
	                hdu3.writeto('tmp3.fits')
        	        hdu4 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[3]/betav_seg))
                	hdu4.writeto('tmp4.fits')
	                hdu5 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[4]/betav_seg))
        	        hdu5.writeto('tmp5.fits')
                	hdu6 = fits.PrimaryHDU(2.5*np.log10(bv0Zs[5]/betav_seg))
	                hdu6.writeto('tmp6.fits')

        	        iraf.ellipse(input='tmp.fits',output='tmp.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
                	iraf.tdump(table='tmp.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV.txt',columns='@colnames.lis')

			iraf.ellipse(input='tmp1.fits',output='tmp1.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
			iraf.tdump(table='tmp1.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV1.txt',columns='@colnames.lis')
	                iraf.ellipse(input='tmp2.fits',output='tmp2.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
        	        iraf.tdump(table='tmp2.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV2.txt',columns='@colnames.lis')
	                iraf.ellipse(input='tmp3.fits',output='tmp3.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
        	        iraf.tdump(table='tmp3.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV3.txt',columns='@colnames.lis')
	                iraf.ellipse(input='tmp4.fits',output='tmp4.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
        	        iraf.tdump(table='tmp4.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV4.txt',columns='@colnames.lis')
	                iraf.ellipse(input='tmp5.fits',output='tmp5.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
        	        iraf.tdump(table='tmp5.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV5.txt',columns='@colnames.lis')
	                iraf.ellipse(input='tmp6.fits',output='tmp6.tab',inter='no',inellip=work_dir+'ellipse/pa_ellip/'+name+'_I1.tab')
        	        iraf.tdump(table='tmp6.tab',datafile=work_dir+'ellipse/pa_ellip/'+name+'_AV6.txt',columns='@colnames.lis')

			######### read ELLIPSE for A_V #########
			ell2=ascii.read(work_dir+'ellipse/pa_ellip/'+name+'_AV.txt')
			h24 = fig.add_axes([0.843,0.39,0.1,0.1])
			h24.plot(ell2['col1']/hlr_pix,ell2['col2'],color='red')
			h24.fill_between(ell2['col1']/hlr_pix,ell2['col2']-ell2['col4'],ell2['col2']+ell2['col4'],color='red',alpha=0.3)
	...
	if int((sha2fig[x]-1)%2):
		copyfile(work_dir+'pdf/'+name+'.tex.pdf','/Users/dhk/Documents/publish/ngcic/figset2_'+str(int((sha2fig[x]-1)/2))+'b.pdf')
	else:
		copyfile(work_dir+'pdf/'+name+'.tex.pdf','/Users/dhk/Documents/publish/ngcic/figset2_'+str(int((sha2fig[x]-1)/2))+'a.pdf')
