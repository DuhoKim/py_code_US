...
for i in range(3):		# for SFH3,4,5
	for j in range(3,6):	# for Z=.0001,.0004,.004,.008,.02,.05
		...
		plt.text(sfh_ttype_bgn[i] + xoff[i][j-3], bv0[i][j] + yoff[i][j-3], r'$\langle$Z$\rangle$$_{\mathrm{M}}$='+Zs[i][j-3]+r'$\,$Z$\odot$', size=9, color=cols_for_Z[j], alpha=1.0)
		plt.text(sfh_ttype_bgn[i] + xoff_noevol[i][j-3] , bv0_noevol[i][j] + yoff_noevol[i][j-3], r'Z='+Zs_noevol[j-3]+r'$\,$Z$\odot$', size=7, alpha=0.3, color='k')

plt.scatter(hubble_coords, hubble_bv0, color='k', label='Av profile  CALIFA')
...
#plt.ylim(0,3.5)
plt.xlabel('Hubble type',size=20)
plt.ylabel(r'$f_{V}/f_{3.6\mu m}$ ($A_V$=0)',size=20)
plt.tick_params(direction='in',labelsize='large',top=True,right=True)
#plt.xticks([-5,0,5,10])
plt.setp(a, xticks=range(7),xticklabels=hubble_label)
fig.savefig('/Users/dhk/Documents/publish/ngcic/betav0_hubble.ps')
