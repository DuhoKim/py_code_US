...
hubble_bv0 = [0.59, 0.69, 0.77, 0.77, 0.59, 0.69, 0.77, 0.77]
...
bv_prof_E1 = np.zeros((31,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_E1[:] = np.nan
bv_prof_S01 = np.zeros((20,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_S01[:] = np.nan
bv_prof_Sa1 = np.zeros((17,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_Sa1[:] = np.nan
bv_prof_Sb1 = np.zeros((40,30))        # For stacking Dust profiles for SFH4 w/ 6 Zs
bv_prof_Sb1[:] = np.nan

bv_prof_E2 = np.zeros((31,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_E2[:] = np.nan
bv_prof_S02 = np.zeros((18,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_S02[:] = np.nan
bv_prof_Sa2 = np.zeros((14,30))	# For stacking Dust profiles for SFH3 w/ 6 Zs
bv_prof_Sa2[:] = np.nan
bv_prof_Sb2 = np.zeros((40,30))        # For stacking Dust profiles for SFH4 w/ 6 Zs
bv_prof_Sb2[:] = np.nan

nan_cnt_E1 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_E2 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_S01 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_S02 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_Sa1 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_Sa2 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_Sb1 = np.zeros(30)        # for counting NaN values in the stack
nan_cnt_Sb2 = np.zeros(30)        # for counting NaN values in the stack
...
for i in np.arange(-0.1, 0.1, 0.01):
    ...
    for j in flag_E1:
        ...
        chi += (av_prof_E1[j] - rosa_AV[0][3+j])**2 / sig**2
    if chi < min_chi[0]:
        min_chi[0] = chi
    ...
    for j in flag_S01:
        ...
        chi += (av_prof_S01[j] - rosa_AV[2][3 + j])**2 / sig**2
    if chi < min_chi[1]:
        min_chi[1] = chi
    ...
    for j in flag_Sa1:
        ...
        chi += (av_prof_Sa1[j] - rosa_AV[4][3 + j])**2 / sig**2
    if chi < min_chi[2]:
        min_chi[2] = chi
    ...
    for j in flag_Sb1:
        ...
        chi += (av_prof_Sb1[j] - rosa_AV[6][3 + j])**2 / sig**2
    if chi < min_chi[3]:
        min_chi[3] = chi
    ...
    for j in flag_E2:
        ...
        chi += (av_prof_E2[j] - rosa_AV[0][3+j])**2 / sig**2
    if chi < min_chi[4]:
        min_chi[4] = chi
    ...
    for j in flag_S02:
        ...
        chi += (av_prof_S02[j] - rosa_AV[2][3 + j])**2 / sig**2
    if chi < min_chi[5]:
        min_chi[5] = chi
    ...
    for j in flag_Sa2:
        ...
        chi += (av_prof_Sa2[j] - rosa_AV[4][3 + j])**2 / sig**2
    if chi < min_chi[6]:
        min_chi[6] = chi
    ...
    for j in flag_Sb2:
        ...
        chi += (av_prof_Sb2[j] - rosa_AV[6][3 + j])**2 / sig**2
    if chi < min_chi[7]:
        min_chi[7] = chi
...
fig.savefig('/Users/dhk/Documents/publish/ngcic_rev/dust_prof_all_T_BT_avg_betav.pdf')
plt.close(fig)
